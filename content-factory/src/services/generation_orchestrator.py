    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            async with AsyncSessionLocal() as db3:
                svc = GenerationJobService(db3)
                job = await svc.get_job(job_id, with_children=False)
                if job and job.status == "running":
                    job.status = "failed"
                    await svc.append_log(
                        job,
                        "Pipeline exited without terminal state; job auto-failed for safety.",
                        "error",
                    )
                    svc.touch(job)
                    await db3.commit()
...
async def _execute_pipeline(db, job_id: uuid.UUID) -> Optional[str]:
    ...
    for step_name, _order in PIPELINE_STEPS:
        ...
        step.progress = 0
        svc.touch(job)
        await svc.append_log(job, f"Step started: {step_name} job_id={job.id} step_name={step_name}")
        await db.commit()
        ...
        try:
            ...
            await svc.append_log(
                job,
                f"Step failed: {step_name} job_id={job.id} step_name={step_name} error={e}",
                "error",
            )
            svc.touch(job)
            await db.commit()
        ...
        _set_job_step_control(job, {step_name: "completed"})
        svc.touch(job)
        await svc.append_log(job, f"Step completed: {step_name} job_id={job.id} step_name={step_name}")
        await db.commit()
        emit(
            "step_complete",
            job_id=str(job_id),
...
def _capped_scene_count(payload: dict[str, Any]) -> int:
    ...
    raw = max(1, raw)
    if settings.GENERATION_DEMO_MODE:
...
def _safe_kie_video_duration(duration: int | None) -> int:
    return max(3, min(int(duration or 5), 5))
...
async def _kie_generate_video_with_retry(
    ...
    safe_duration = _safe_kie_video_duration(duration)
    r = await kie.generate_video(prompt, duration=safe_duration, trace=trace)
...
async def _step_video_generation(db, svc: GenerationJobService, job: GenerationJob, step: GenerationStep) -> None:
    ...
    async def run_kie_for_scene(sc: GenerationScene) -> dict[str, Any]:
        async with sem:
            ...
            dur = _safe_kie_video_duration(sc.duration)
            row: dict[str, Any] = {
                "scene_index": sc.scene_index,
                "scene_id": sid,
    ...
    if v_ok < required:
        ...
        if not settings.GENERATION_DEMO_MODE:
            raise RuntimeError(
...
async def generate_scene_preview(
    ...
    if kind == "video":
        ...
        if sc.video_url:
            ...
            vres = await kie.generate_video(
                prompt,
                duration=_safe_kie_video_duration(3),
                trace=preview_trace,
            )
            url = vres.get("url")
            if url == "ERROR: INSUFFICIENT_CREDITS":
...
async def _step_distribution(db, svc: GenerationJobService, job: GenerationJob, step: GenerationStep) -> None:
    ...
    if bool(job.distribution_sent):
        await svc.append_log(
            job,
            f"Step skipped (already sent): distribution job_id={job.id} step_name=distribution",
            "info",
        )
        emit(
            "distribution_skip_already_sent",
            job_id=gid,
            step="distribution",
            outcome="skipped",
            reason="distribution_sent_true",
        )
        step.progress = 100
        job.progress = 100
        return
