          ...
          <div className="rounded-2xl border bg-card shadow-sm overflow-hidden">
            <div className="p-5 border-b bg-muted/10 flex items-center justify-between">
              <h3 className="font-bold flex items-center gap-2">
                ...
                {text.analytics.latestRoi}
              </h3>
              <span className="text-[10px] uppercase font-bold text-muted-foreground">{text.analytics.recent50}</span>
            </div>
            <div className="divide-y">
              {loading ? (
                <div className="p-20 flex flex-col items-center justify-center text-muted-foreground">
                  <Loader2 className="w-8 h-8 animate-spin mb-2" />
                  <p className="text-sm">{text.analytics.aggregating}</p>
                </div>
              ) : posts.length === 0 ? (
                <div className="p-10 text-center text-muted-foreground">{text.analytics.noPublications}</div>
              ) : (
                posts.map((post) => (
                  <div key={post.id} className="p-4 hover:bg-muted/10 transition-colors flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-6 shrink-0">
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-muted-foreground uppercase font-medium">{text.analytics.likes}</span>
                      </div>
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[10px] text-muted-foreground uppercase font-medium">{text.analytics.comments}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        ...
                        <span className="text-[9px] text-muted-foreground mt-1">{text.analytics.engagement}</span>
                      </div>
                    </div>
                  </div>
                ))
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border bg-card shadow-sm overflow-hidden p-6 bg-gradient-to-br from-indigo-50/50 to-transparent">
            <h4 className="font-bold flex items-center gap-2 mb-4">
              ...
              {text.analytics.topGlobalContent}
            </h4>
            ...
            <button className="w-full mt-6 py-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors flex items-center justify-center gap-2">
              {text.analytics.viewContentCloud}
          ...
          <div className="rounded-2xl border bg-card p-6 shadow-sm">
            ...
            <p className="text-[10px] text-muted-foreground mt-4 text-center">{text.analytics.heatmapBasis}</p>
          </div>
        </div>
      </div>
    </div>
